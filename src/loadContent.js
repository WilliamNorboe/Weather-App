...
async function loadImage() {
  ...
  let response = await fetch(
    "https://api.giphy.com/v1/gifs/translate?api_key=rj5Y4vSo1SUlJFJwJ2fWse13jRvpyZfQ&s=" +
      search,
    { mode: "cors" }
  );
...
async function loadWeather() {
  ...
  let response = await fetch(
    "https://api.weatherapi.com/v1/current.json?key=7c5bf67a3b524d7084a183409233006&q=" +
      search,
    { mode: "cors" }
  );
  ...
  tempInfoDiv.textContent =
    currentTemp + " in " + weatherData.location.name + " Right Now";
}
